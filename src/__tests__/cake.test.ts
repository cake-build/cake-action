...
describe('When running a script successfully using the global Cake tool', () => {
  ...
  test('it should run the global dotnet-cake tool with the specified parameters', async () => {
    await cake.runScript('script.cake', undefined, new CakeArgument('param', 'arg'), new CakeSwitch('switch'));
    expect(fakeExec).toHaveBeenCalledWith('/usr/bin/dotnet-cake', ['script.cake', '--param=arg', '--switch']);
  });

  test('it should run the global dotnet-cake tool without any invalid parameters', async () => {
    await cake.runScript('script.cake', undefined, new CakeArgument('', ''), new CakeSwitch('switch'));
    expect(fakeExec).toHaveBeenCalledWith('/usr/bin/dotnet-cake', ['script.cake', '--switch']);
  });
});
...
describe('When running a script successfully using the local Cake tool', () => {
  ...
  test('it should run the local dotnet-cake tool with the specified parameters', async () => {
    ...
    expect(fakeExec).toHaveBeenCalledWith(pathToLocalTool, ['script.cake', '--param=arg', '--switch']);
  });
  ...
  test('it should run the local dotnet-cake tool without any invalid parameters', async () => {
    ...
    expect(fakeExec).toHaveBeenCalledWith(pathToLocalTool, ['script.cake', '--switch']);
  });
});
...
describe('When failing to bootstrap a script using the local Cake tool', () => {
  ...
  test('it should throw an error containing the exit code', async () => {
    await expect(cake.bootstrapScript('script.cake', new CakeToolSettings(new ToolsDirectory()))).rejects.toThrow(
      '-21'
    );
  });
});
...
describe('When running a script successfully using the tool manifest', () => {
  ...
  test('it should run the local dotnet-cake tool with the specified parameters', async () => {
    ...
    expect(fakeExec).toHaveBeenCalledWith(dotnetManifestCake, ['script.cake', '--param=arg', '--switch']);
  });
  ...
  test('it should run the local dotnet-cake tool without any invalid parameters', async () => {
    ...
    expect(fakeExec).toHaveBeenCalledWith(dotnetManifestCake, ['script.cake', '--switch']);
  });
});
...
describe('When running a Cake Frosting project successfully', () => {
  ...
  test('it should run with the default non-required parameters', async () => {
    ...
    expect(fakeExec).toHaveBeenCalledWith(dotnetRun, [
      '--project',
      pathToCsprojFile,
      '--no-launch-profile',
      '--verbosity',
...
describe('When running a Cake C# file successfully', () => {
  ...
  test('it should run with the specified parameters', async () => {
    await cake.runFile(pathToCsFile, fakeToolsDirectory, new CakeArgument('param', 'arg'), new CakeSwitch('switch'));
